...
namespace EInkLayout {
    ...
    constexpr int16_t BATTERY_ICON_WIDTH = 20;   // body plus nub
    constexpr int16_t BATTERY_ICON_HEIGHT = 11;
...
namespace EInkWidgets {
    ...
    inline void drawBatteryIcon(GFXcanvas1 &g, const int16_t x, const int16_t y, const int16_t percent) {
        constexpr int16_t bodyWidth = EInkLayout::BATTERY_ICON_WIDTH - 2;
        g.drawRect(x, y, bodyWidth, EInkLayout::BATTERY_ICON_HEIGHT, INK);
        g.fillRect(x + bodyWidth, y + 3, 2, EInkLayout::BATTERY_ICON_HEIGHT - 6, INK);

        const int16_t filled = percent > 80 ? 3 : (percent > 50 ? 2 : (percent > 20 ? 1 : 0));

        // 2 px inside the 1 px outline: three 4 px bars with 1 px gaps span the
        // remaining 14 px exactly.
        constexpr int16_t barWidth = 4;
        constexpr int16_t barHeight = EInkLayout::BATTERY_ICON_HEIGHT - 4;
        for (int16_t i = 0; i < EInkLayout::BATTERY_BARS; i++) {
            const int16_t barX = x + 2 + i * (barWidth + 1);
            if (i < filled) {
    ...
    // The battery icon plus its percent, centred as one block. 12 pt, because this
    // is the footer's headline number.
    inline void drawBatteryReading(GFXcanvas1 &g, const int16_t baseline, const int16_t percent) {
        ...
        drawBatteryIcon(g, x, baseline - EInkLayout::BATTERY_ICON_HEIGHT - 2, percent);
